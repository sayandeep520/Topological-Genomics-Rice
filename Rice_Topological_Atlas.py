...
plt.tight_layout()
plt.savefig(os.path.join(results_dir, 'pca_plot.png')) # Save the plot
plt.show()
...
elif pure_total_original == 0:
    pure_counts_normalized_list = [0] * len(allele_keys)
else: # Handle case of negative counts if somehow possible
    pure_counts_normalized_list = [0] * len(allele_keys) # Or raise an error
...
# This variable `haplotypes` is not defined previously, defining for mock purposes.
haplotypes = [str(x) for x in G_clean[peak_idx, :].T]
...
plt.savefig(os.path.join(results_dir, 'ld_triangle_plot.png')) # Save the plot
plt.show()
